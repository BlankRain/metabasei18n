    ...
    render () {
        ...
        return (
            <div className="flex flex-column spread p4">
                <h2 className="my2">自定义该 {this.getChartTypeName()}</h2>
                ...
                <div className="pt1">
                  <a className={cx("Button Button--primary", { disabled: !isDirty })} onClick={() => this.onDone()} data-metabase-event="Chart Settings;Done">完成</a>
                  <a className="text-grey-2 ml2" onClick={onClose} data-metabase-event="Chart Settings;Cancel">取消</a>
                  { !_.isEqual(this.state.settings, {}) &&
                      <a className="Button Button--warning float-right" onClick={this.onResetSettings} data-metabase-event="Chart Settings;Reset">恢复默认</a>
                  }
                </div>
            </div>
        );
