...
function DeleteGroupModal({ group, onConfirm = () => {} , onClose = () => {} }) {
    return (
        <ModalContent title="移除该组?" onClose={onClose}>
            <p className="px4 pb4">
                你确定? 所有的该组成员将会失去该组拥有的权限,并且,这个是不可恢复的.
            </p>
...
function GroupsTable({ groups, text, groupBeingEdited, showAddGroupRow, onAddGroupCanceled, onAddGroupCreateButtonClicked, onAddGroupTextChanged,
    ...
    return (
        <AdminContentTable columnTitles={["组名", "成员"]}>
            {showAddGroupRow ? (
    ...
    render() {
        ...
        return (
            <AdminPaneLayout
                ...
                description="你可以使用小组来控制用户权限. 把用户加进小组,然后给小组设置访问权限. 管理员组和所有用户组是特殊的组，不可移除."
            >
                <GroupsTable
                    groups={groups}
